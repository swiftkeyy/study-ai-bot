...
async def _open_user_section(message: Message, state: FSMContext, button_text: str) -> None:
    ...
    if normalized_button == "решить задачу":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "написать текст":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "угадай оценку":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "сделай умнее":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "шпора по фото":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "палится ли ai":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "ввести промокод":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "поддержка":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "реферальная программа":
        if await deny_if_blocked_message(message):
            return
    ...
    if normalized_button == "полезные материалы":
        if await deny_if_blocked_message(message):
            return
