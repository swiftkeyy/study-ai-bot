...
def run_quick_fix() -> None:
    ...
    Можно запустить так:
        python /app/db.py
    """
    db_path = DB_PATH or "/app/data/bot.db"
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        if "is_banned" in user_cols:
            if "ban_reason" in user_cols:
                cur = conn.execute(
                    "UPDATE users SET is_banned = 0, ban_reason = NULL WHERE COALESCE(is_banned, 0) != 0"
                )
            else:
                cur = conn.execute(
                    "UPDATE users SET is_banned = 0 WHERE COALESCE(is_banned, 0) != 0"
                )
